...
namespace ApiProject.WebUI.Controllers
{
    public class ReservationController : Controller
    {
        ...
        public async Task<IActionResult> ReservationList()
        {
            ...
            var responseMessage = await client.GetAsync("https://localhost:7123/api/Reservations");
        ...
        {
            var client = _httpClientFactory.CreateClient();
            await client.DeleteAsync("https://localhost:7123/api/Reservations?id=" + id);
            return RedirectToAction("ReservationList");
        }
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.GetAsync("https://localhost:7123/api/Reservations/GetReservation?id=" + id);
            var jsonData = await responseMessage.Content.ReadAsStringAsync();
        ...
        public async Task<IActionResult> UpdateReservation(UpdateReservationDto updateReservationDto)
        {
            ...
            StringContent stringContent = new StringContent(jsonData, Encoding.UTF8, "application/json");
            var responseMessage = await client.PutAsync("https://localhost:7123/api/Reservations", stringContent);
            return RedirectToAction("ReservationList");
        }
    }
}
